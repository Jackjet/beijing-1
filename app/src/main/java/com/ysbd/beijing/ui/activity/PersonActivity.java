    ...
    private void getUserId() {
        new Thread() {
            ...
            public void run() {
                ...
                int count = 0;
                do {
                    s = WebServiceUtils.getInstance().getUserInfo(userId);
                } while (TextUtils.isEmpty(s) && count++ < 10);//如果没获取数据尝试多次获取直到获取数据或者获取次数到达10次
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 0:
                    try {
                        List<PersonBean> personBean = new Gson().fromJson(msg.obj.toString(), new TypeToken<List<PersonBean>>() {
                        }.getType());
