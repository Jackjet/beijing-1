package com.ysbd.beijing.ui.bean.form;

import com.ysbd.beijing.ui.bean.CommentBean;
import com.ysbd.beijing.ui.bean.CurrentCommentBean;
import com.ysbd.beijing.ui.bean.MenuBean;

import java.util.List;

/**
 * Created by lcjing on 2018/8/20.
 */

public class ZhibiaowenBean extends BaseFormBean{

    /**
     * bangongshifenshu : 0
     * bangongshiliucheng :
     * baoguanqixian : 长期
     * baomiqixian :
     * biaoti : 测试4
     * biaotou :
     * chaosong : 市审计局
     * chengwenriqi : 2014-12-31 00:00:00.0
     * chushifenshu : 4
     * chushihegao :
     * chushihuiqian :
     * chushiliucheng :
     * chushi_zi : 预
     * chuzhanghegao :
     * fawenleixing :
     * fenshu : 4
     * formguid :
     * gongkaishuxing : 主动公开
     * guifanwenjian :
     * hao :
     * hiddenaaa :
     * hqzbw_chushi_hao : 55
     * huanji :
     * isdengzhang :
     * issave :
     * jiaodui :
     * jiaoduidianhua :
     * jiaoduiriqi :
     * jiyaotozhibiao :
     * juneihuiqian :
     * jutozhi :
     * juwaiqianfa :
     * lookjyydwurl :
     * lookydwurl :
     * miji : 非密
     * mishushenhe :
     * nian : 2015
     * nigao : 张久亮
     * nigaodanwei : 预算处(预算编审中心)
     * nigaodianhua : 4
     * nigaoriqi : 2014-12-25 00:00:00.0
     * qianfa :
     * riseword :
     * shitozhi :
     * tiqianxiada :
     * workflowinstance_guid : {0A2FF324-0000-0000-7F62-7A8100000001}
     * xianbanshijian : 2015-01-04 00:00:00.0
     * ydwwenhao :
     * yesno :
     * yijianlan :
     * yinzhi : 王燕
     * ysdayin :
     * ysjieshu :
     * yuanyincontent : 无
     * yuebantozhibiao :
     * zhibiaochaxun :
     * zhurenshenhe :
     * zhusong :
     * zhusongselect :
     * zhutici :
     * zhutozhi :
     * zi : 预
     */

    private String bangongshifenshu;//办公室份数
    private String bangongshiliucheng;//办公室流程
    private String baoguanqixian;//保管期限
    private String baomiqixian;//保密期限
    private String biaoti;//标题
    private String biaotou;//表头
    private String chaosong;//抄送
    private String chengwenriqi;//成文日期
    private String chushifenshu;//处室份数
    private String chushihegao;//处室核稿
    private String chushihuiqian;//处室会签
    private String chushiliucheng;//处室流程
    private String chushi_zi;//处室 字
    private String chuzhanghegao;//处长核稿
    private String fawenleixing;//发文类型
    private String fenshu;//份数
    private String formguid;//
    private String gongkaishuxing;//公开属性
    private String guifanwenjian;//规范文件
    private String hao;//号
    private String hiddenaaa;//
    private String hqzbw_chushi_hao;// 主办文 处室号
    private String huanji;//缓急
    private String isdengzhang;//
    private String issave;//
    private String jiaodui;//
    private String jiaoduidianhua;//
    private String jiaoduiriqi;//
    private String jiyaotozhibiao;//
    private String juneihuiqian;//
    private String jutozhi;//
    private String juwaiqianfa;//
    private String lookjyydwurl;//
    private String lookydwurl;//
    private String miji;//
    private String mishushenhe;//
    private String nian;//
    private String nigao;//
    private String nigaodanwei;//
    private String nigaodianhua;//
    private String nigaoriqi;//
    private String qianfa;//
    private String riseword;//
    private String shitozhi;//
    private String tiqianxiada;//
    private String workflowinstance_guid;//
    private String xianbanshijian;//
    private String ydwwenhao;//
    private String yesno;//
    private String yijianlan;//
    private String yinzhi;//
    private String ysdayin;//
    private String ysjieshu;//
    private String yuanyincontent;//
    private String yuebantozhibiao;//
    private String zhibiaochaxun;//
    private String zhurenshenhe;//
    private String zhusong;//
    private String zhusongselect;//
    private String zhutici;//
    private String zhutozhi;//
    private String zi;//



    public String getBangongshifenshu() {
        return bangongshifenshu;
    }

    public void setBangongshifenshu(String bangongshifenshu) {
        this.bangongshifenshu = bangongshifenshu;
    }

    public String getBangongshiliucheng() {
        return bangongshiliucheng;
    }

    public void setBangongshiliucheng(String bangongshiliucheng) {
        this.bangongshiliucheng = bangongshiliucheng;
    }

    public String getBaoguanqixian() {
        return baoguanqixian;
    }

    public void setBaoguanqixian(String baoguanqixian) {
        this.baoguanqixian = baoguanqixian;
    }

    public String getBaomiqixian() {
        return baomiqixian;
    }

    public void setBaomiqixian(String baomiqixian) {
        this.baomiqixian = baomiqixian;
    }

    public String getBiaoti() {
        return biaoti;
    }

    public void setBiaoti(String biaoti) {
        this.biaoti = biaoti;
    }

    public String getBiaotou() {
        return biaotou;
    }

    public void setBiaotou(String biaotou) {
        this.biaotou = biaotou;
    }

    public String getChaosong() {
        return chaosong;
    }

    public void setChaosong(String chaosong) {
        this.chaosong = chaosong;
    }

    public String getChengwenriqi() {
        return chengwenriqi;
    }

    public void setChengwenriqi(String chengwenriqi) {
        this.chengwenriqi = chengwenriqi;
    }

    public String getChushifenshu() {
        return chushifenshu;
    }

    public void setChushifenshu(String chushifenshu) {
        this.chushifenshu = chushifenshu;
    }

    public String getChushihegao() {
        return chushihegao;
    }

    public void setChushihegao(String chushihegao) {
        this.chushihegao = chushihegao;
    }

    public String getChushihuiqian() {
        return chushihuiqian;
    }

    public void setChushihuiqian(String chushihuiqian) {
        this.chushihuiqian = chushihuiqian;
    }

    public String getChushiliucheng() {
        return chushiliucheng;
    }

    public void setChushiliucheng(String chushiliucheng) {
        this.chushiliucheng = chushiliucheng;
    }

    public String getChushi_zi() {
        return chushi_zi;
    }

    public void setChushi_zi(String chushi_zi) {
        this.chushi_zi = chushi_zi;
    }

    public String getChuzhanghegao() {
        return chuzhanghegao;
    }

    public void setChuzhanghegao(String chuzhanghegao) {
        this.chuzhanghegao = chuzhanghegao;
    }

    public String getFawenleixing() {
        return fawenleixing;
    }

    public void setFawenleixing(String fawenleixing) {
        this.fawenleixing = fawenleixing;
    }

    public String getFenshu() {
        return fenshu;
    }

    public void setFenshu(String fenshu) {
        this.fenshu = fenshu;
    }

    public String getFormguid() {
        return formguid;
    }

    public void setFormguid(String formguid) {
        this.formguid = formguid;
    }

    public String getGongkaishuxing() {
        return gongkaishuxing;
    }

    public void setGongkaishuxing(String gongkaishuxing) {
        this.gongkaishuxing = gongkaishuxing;
    }

    public String getGuifanwenjian() {
        return guifanwenjian;
    }

    public void setGuifanwenjian(String guifanwenjian) {
        this.guifanwenjian = guifanwenjian;
    }

    public String getHao() {
        return hao;
    }

    public void setHao(String hao) {
        this.hao = hao;
    }

    public String getHiddenaaa() {
        return hiddenaaa;
    }

    public void setHiddenaaa(String hiddenaaa) {
        this.hiddenaaa = hiddenaaa;
    }

    public String getHqzbw_chushi_hao() {
        return hqzbw_chushi_hao;
    }

    public void setHqzbw_chushi_hao(String hqzbw_chushi_hao) {
        this.hqzbw_chushi_hao = hqzbw_chushi_hao;
    }

    public String getHuanji() {
        return huanji;
    }

    public void setHuanji(String huanji) {
        this.huanji = huanji;
    }

    public String getIsdengzhang() {
        return isdengzhang;
    }

    public void setIsdengzhang(String isdengzhang) {
        this.isdengzhang = isdengzhang;
    }

    public String getIssave() {
        return issave;
    }

    public void setIssave(String issave) {
        this.issave = issave;
    }

    public String getJiaodui() {
        return jiaodui;
    }

    public void setJiaodui(String jiaodui) {
        this.jiaodui = jiaodui;
    }

    public String getJiaoduidianhua() {
        return jiaoduidianhua;
    }

    public void setJiaoduidianhua(String jiaoduidianhua) {
        this.jiaoduidianhua = jiaoduidianhua;
    }

    public String getJiaoduiriqi() {
        return jiaoduiriqi;
    }

    public void setJiaoduiriqi(String jiaoduiriqi) {
        this.jiaoduiriqi = jiaoduiriqi;
    }

    public String getJiyaotozhibiao() {
        return jiyaotozhibiao;
    }

    public void setJiyaotozhibiao(String jiyaotozhibiao) {
        this.jiyaotozhibiao = jiyaotozhibiao;
    }

    public String getJuneihuiqian() {
        return juneihuiqian;
    }

    public void setJuneihuiqian(String juneihuiqian) {
        this.juneihuiqian = juneihuiqian;
    }

    public String getJutozhi() {
        return jutozhi;
    }

    public void setJutozhi(String jutozhi) {
        this.jutozhi = jutozhi;
    }

    public String getJuwaiqianfa() {
        return juwaiqianfa;
    }

    public void setJuwaiqianfa(String juwaiqianfa) {
        this.juwaiqianfa = juwaiqianfa;
    }

    public String getLookjyydwurl() {
        return lookjyydwurl;
    }

    public void setLookjyydwurl(String lookjyydwurl) {
        this.lookjyydwurl = lookjyydwurl;
    }

    public String getLookydwurl() {
        return lookydwurl;
    }

    public void setLookydwurl(String lookydwurl) {
        this.lookydwurl = lookydwurl;
    }

    public String getMiji() {
        return miji;
    }

    public void setMiji(String miji) {
        this.miji = miji;
    }

    public String getMishushenhe() {
        return mishushenhe;
    }

    public void setMishushenhe(String mishushenhe) {
        this.mishushenhe = mishushenhe;
    }

    public String getNian() {
        return nian;
    }

    public void setNian(String nian) {
        this.nian = nian;
    }

    public String getNigao() {
        return nigao;
    }

    public void setNigao(String nigao) {
        this.nigao = nigao;
    }

    public String getNigaodanwei() {
        return nigaodanwei;
    }

    public void setNigaodanwei(String nigaodanwei) {
        this.nigaodanwei = nigaodanwei;
    }

    public String getNigaodianhua() {
        return nigaodianhua;
    }

    public void setNigaodianhua(String nigaodianhua) {
        this.nigaodianhua = nigaodianhua;
    }

    public String getNigaoriqi() {
        return nigaoriqi;
    }

    public void setNigaoriqi(String nigaoriqi) {
        this.nigaoriqi = nigaoriqi;
    }

    public String getQianfa() {
        return qianfa;
    }

    public void setQianfa(String qianfa) {
        this.qianfa = qianfa;
    }

    public String getRiseword() {
        return riseword;
    }

    public void setRiseword(String riseword) {
        this.riseword = riseword;
    }

    public String getShitozhi() {
        return shitozhi;
    }

    public void setShitozhi(String shitozhi) {
        this.shitozhi = shitozhi;
    }

    public String getTiqianxiada() {
        return tiqianxiada;
    }

    public void setTiqianxiada(String tiqianxiada) {
        this.tiqianxiada = tiqianxiada;
    }

    public String getWorkflowinstance_guid() {
        return workflowinstance_guid;
    }

    public void setWorkflowinstance_guid(String workflowinstance_guid) {
        this.workflowinstance_guid = workflowinstance_guid;
    }

    public String getXianbanshijian() {
        return xianbanshijian;
    }

    public void setXianbanshijian(String xianbanshijian) {
        this.xianbanshijian = xianbanshijian;
    }

    public String getYdwwenhao() {
        return ydwwenhao;
    }

    public void setYdwwenhao(String ydwwenhao) {
        this.ydwwenhao = ydwwenhao;
    }

    public String getYesno() {
        return yesno;
    }

    public void setYesno(String yesno) {
        this.yesno = yesno;
    }

    public String getYijianlan() {
        return yijianlan;
    }

    public void setYijianlan(String yijianlan) {
        this.yijianlan = yijianlan;
    }

    public String getYinzhi() {
        return yinzhi;
    }

    public void setYinzhi(String yinzhi) {
        this.yinzhi = yinzhi;
    }

    public String getYsdayin() {
        return ysdayin;
    }

    public void setYsdayin(String ysdayin) {
        this.ysdayin = ysdayin;
    }

    public String getYsjieshu() {
        return ysjieshu;
    }

    public void setYsjieshu(String ysjieshu) {
        this.ysjieshu = ysjieshu;
    }

    public String getYuanyincontent() {
        return yuanyincontent;
    }

    public void setYuanyincontent(String yuanyincontent) {
        this.yuanyincontent = yuanyincontent;
    }

    public String getYuebantozhibiao() {
        return yuebantozhibiao;
    }

    public void setYuebantozhibiao(String yuebantozhibiao) {
        this.yuebantozhibiao = yuebantozhibiao;
    }

    public String getZhibiaochaxun() {
        return zhibiaochaxun;
    }

    public void setZhibiaochaxun(String zhibiaochaxun) {
        this.zhibiaochaxun = zhibiaochaxun;
    }

    public String getZhurenshenhe() {
        return zhurenshenhe;
    }

    public void setZhurenshenhe(String zhurenshenhe) {
        this.zhurenshenhe = zhurenshenhe;
    }

    public String getZhusong() {
        return zhusong;
    }

    public void setZhusong(String zhusong) {
        this.zhusong = zhusong;
    }

    public String getZhusongselect() {
        return zhusongselect;
    }

    public void setZhusongselect(String zhusongselect) {
        this.zhusongselect = zhusongselect;
    }

    public String getZhutici() {
        return zhutici;
    }

    public void setZhutici(String zhutici) {
        this.zhutici = zhutici;
    }

    public String getZhutozhi() {
        return zhutozhi;
    }

    public void setZhutozhi(String zhutozhi) {
        this.zhutozhi = zhutozhi;
    }

    public String getZi() {
        return zi;
    }

    public void setZi(String zi) {
        this.zi = zi;
    }






}
