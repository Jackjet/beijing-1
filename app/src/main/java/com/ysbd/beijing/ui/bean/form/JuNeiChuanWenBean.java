package com.ysbd.beijing.ui.bean.form;

import com.google.gson.annotations.SerializedName;

import java.util.List;

/**
 * Created by lcjing on 2018/8/27.
 */

public class JuNeiChuanWenBean extends BaseFormBean {

    /**
     * baoguanqixian :
     * beizhu :
     * chengbanren : 梁策(信息处（信息中心）)
     * chengbaoneirong :
     * chengsong :
     * chuanyueguocheng :
     * chushimingcheng : 信息处（信息中心）
     * chushi_zi : 信息
     * chuzhangpishi :
     * chuzhangqianzi :
     * dianhua : 0000
     * formguid :
     * guidangren :
     * guidangshijian :
     * hao : 303
     * jianyaoqingkuang :
     * julingdaopishi :
     * juneitojieyu :
     * juneitoyiban :
     * jutobian :
     * jutozhi :
     * miji :
     * nian : 2016
     * qicaochushi :
     * qitarenyijian :
     * riseword :
     * shifoufawen :
     * shouwenriqi : 2016-04-14
     * wenjianmingcheng : 测试时间
     * workflowinstance_guid : {0A2FF324-0000-0000-11F0-8FB80000000C}
     * workflow_sub :
     * xianbanshijian : 2016-05-09&#32;00:00:00.0
     * xiebanyijian :
     * zhongdianducha :
     * zhubanchushi : 信息处（信息中心）
     * attachment : []
     * actors : []
     */

    private String baoguanqixian;
    private String beizhu;
    private String chengbanren;
    private String chengbaoneirong;
    private String chengsong;
    private String chuanyueguocheng;
    private String chushimingcheng;
    private String chushi_zi;
    private String chuzhangpishi;
    private String chuzhangqianzi;
    private String dianhua;
    private String formguid;
    private String guidangren;
    private String guidangshijian;
    private String hao;
    private String jianyaoqingkuang;
    private String julingdaopishi;
    private String juneitojieyu;
    private String juneitoyiban;
    private String jutobian;
    private String jutozhi;
    private String miji;
    private String nian;
    private String qicaochushi;
    private String qitarenyijian;
    private String riseword;
    private String shifoufawen;
    private String shouwenriqi;
    private String wenjianmingcheng;
    private String workflowinstance_guid;
    private String workflow_sub;
    private String xianbanshijian;
    private String xiebanyijian;
    private String zhongdianducha;
    private String zhubanchushi;




    public String getBaoguanqixian() {
        return baoguanqixian;
    }

    public void setBaoguanqixian(String baoguanqixian) {
        this.baoguanqixian = baoguanqixian;
    }

    public String getBeizhu() {
        return beizhu;
    }

    public void setBeizhu(String beizhu) {
        this.beizhu = beizhu;
    }

    public String getChengbanren() {
        return chengbanren;
    }

    public void setChengbanren(String chengbanren) {
        this.chengbanren = chengbanren;
    }

    public String getChengbaoneirong() {
        return chengbaoneirong;
    }

    public void setChengbaoneirong(String chengbaoneirong) {
        this.chengbaoneirong = chengbaoneirong;
    }

    public String getChengsong() {
        return chengsong;
    }

    public void setChengsong(String chengsong) {
        this.chengsong = chengsong;
    }

    public String getChuanyueguocheng() {
        return chuanyueguocheng;
    }

    public void setChuanyueguocheng(String chuanyueguocheng) {
        this.chuanyueguocheng = chuanyueguocheng;
    }

    public String getChushimingcheng() {
        return chushimingcheng;
    }

    public void setChushimingcheng(String chushimingcheng) {
        this.chushimingcheng = chushimingcheng;
    }

    public String getChushi_zi() {
        return chushi_zi;
    }

    public void setChushi_zi(String chushi_zi) {
        this.chushi_zi = chushi_zi;
    }

    public String getChuzhangpishi() {
        return chuzhangpishi;
    }

    public void setChuzhangpishi(String chuzhangpishi) {
        this.chuzhangpishi = chuzhangpishi;
    }

    public String getChuzhangqianzi() {
        return chuzhangqianzi;
    }

    public void setChuzhangqianzi(String chuzhangqianzi) {
        this.chuzhangqianzi = chuzhangqianzi;
    }

    public String getDianhua() {
        return dianhua;
    }

    public void setDianhua(String dianhua) {
        this.dianhua = dianhua;
    }

    public String getFormguid() {
        return formguid;
    }

    public void setFormguid(String formguid) {
        this.formguid = formguid;
    }

    public String getGuidangren() {
        return guidangren;
    }

    public void setGuidangren(String guidangren) {
        this.guidangren = guidangren;
    }

    public String getGuidangshijian() {
        return guidangshijian;
    }

    public void setGuidangshijian(String guidangshijian) {
        this.guidangshijian = guidangshijian;
    }

    public String getHao() {
        return hao;
    }

    public void setHao(String hao) {
        this.hao = hao;
    }

    public String getJianyaoqingkuang() {
        return jianyaoqingkuang;
    }

    public void setJianyaoqingkuang(String jianyaoqingkuang) {
        this.jianyaoqingkuang = jianyaoqingkuang;
    }

    public String getJulingdaopishi() {
        return julingdaopishi;
    }

    public void setJulingdaopishi(String julingdaopishi) {
        this.julingdaopishi = julingdaopishi;
    }

    public String getJuneitojieyu() {
        return juneitojieyu;
    }

    public void setJuneitojieyu(String juneitojieyu) {
        this.juneitojieyu = juneitojieyu;
    }

    public String getJuneitoyiban() {
        return juneitoyiban;
    }

    public void setJuneitoyiban(String juneitoyiban) {
        this.juneitoyiban = juneitoyiban;
    }

    public String getJutobian() {
        return jutobian;
    }

    public void setJutobian(String jutobian) {
        this.jutobian = jutobian;
    }

    public String getJutozhi() {
        return jutozhi;
    }

    public void setJutozhi(String jutozhi) {
        this.jutozhi = jutozhi;
    }

    public String getMiji() {
        return miji;
    }

    public void setMiji(String miji) {
        this.miji = miji;
    }

    public String getNian() {
        return nian;
    }

    public void setNian(String nian) {
        this.nian = nian;
    }

    public String getQicaochushi() {
        return qicaochushi;
    }

    public void setQicaochushi(String qicaochushi) {
        this.qicaochushi = qicaochushi;
    }

    public String getQitarenyijian() {
        return qitarenyijian;
    }

    public void setQitarenyijian(String qitarenyijian) {
        this.qitarenyijian = qitarenyijian;
    }

    public String getRiseword() {
        return riseword;
    }

    public void setRiseword(String riseword) {
        this.riseword = riseword;
    }

    public String getShifoufawen() {
        return shifoufawen;
    }

    public void setShifoufawen(String shifoufawen) {
        this.shifoufawen = shifoufawen;
    }

    public String getShouwenriqi() {
        return shouwenriqi;
    }

    public void setShouwenriqi(String shouwenriqi) {
        this.shouwenriqi = shouwenriqi;
    }

    public String getWenjianmingcheng() {
        return wenjianmingcheng;
    }

    public void setWenjianmingcheng(String wenjianmingcheng) {
        this.wenjianmingcheng = wenjianmingcheng;
    }

    public String getWorkflowinstance_guid() {
        return workflowinstance_guid;
    }

    public void setWorkflowinstance_guid(String workflowinstance_guid) {
        this.workflowinstance_guid = workflowinstance_guid;
    }

    public String getWorkflow_sub() {
        return workflow_sub;
    }

    public void setWorkflow_sub(String workflow_sub) {
        this.workflow_sub = workflow_sub;
    }

    public String getXianbanshijian() {
        return xianbanshijian;
    }

    public void setXianbanshijian(String xianbanshijian) {
        this.xianbanshijian = xianbanshijian;
    }

    public String getXiebanyijian() {
        return xiebanyijian;
    }

    public void setXiebanyijian(String xiebanyijian) {
        this.xiebanyijian = xiebanyijian;
    }

    public String getZhongdianducha() {
        return zhongdianducha;
    }

    public void setZhongdianducha(String zhongdianducha) {
        this.zhongdianducha = zhongdianducha;
    }

    public String getZhubanchushi() {
        return zhubanchushi;
    }

    public void setZhubanchushi(String zhubanchushi) {
        this.zhubanchushi = zhubanchushi;
    }

}
