...
import com.mcxtzhang.indexlib.IndexBar.PolyphoneUtil;
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (indexPinyinBean.isNeedToPinyin()) {
                ...
                for (int i1 = 0; i1 < target.length(); i1++) {
                    char c = target.charAt(i1);
                    if (i1 == 0 && PolyphoneUtil.isPolyphone(String.valueOf(c))) {
                        pySb.append(PolyphoneUtil.getPinYin(String.valueOf(c)));
                    } else {
                        pySb.append(Pinyin.toPinyin(c).toUpperCase());
                    }
